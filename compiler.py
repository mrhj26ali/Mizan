...
def get_target_machine():
    ...
    # ✅ REVERTED: We MUST use the default MSVC triple so LLVM generates 
    # the Windows x64 calling convention (%rcx, %rdx) to match the MinGW C runtime.
    # The __chkstk issue is already permanently solved by using a global buffer.
    print(f"🎯 المعمارية: {triple}")
    target = llvm.Target.from_default_triple()
    
    # ✅ FIXED: Removed the invalid 'triple=triple' kwarg
...
def compile_file(source_path: str, run_after=False, visualize=False):
    ...
    # ✅ REVERTED: Let IRGenerator use the default Windows triple
    irgen  = IRGenerator(semantic_symbols=analyzer.current_scope.all_symbols())
    ...
    if args.all:
        ...
        for prog in programs:
            if os.path.isfile(prog):
                try:
                    compile_file(prog, run_after=args.run, visualize=args.ast)
                except Exception:
                    print(f"❌ فشل: {prog}"); traceback.print_exc()
    else:
        try:
            compile_file(args.source, run_after=args.run, visualize=args.ast)
        except Exception:
            traceback.print_exc()
